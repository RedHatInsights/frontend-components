...
export const defaultColumns = [
  ...
  },
  {
    ...
    title: 'Last seen',
    // eslint-disable-next-line react/display-name
    renderFunc: (value, _id, { culled_timestamp: culled, stale_warning_timestamp: staleWarn, stale_timestamp: stale, insights_id: insightsId }) => {
...
function entitiesPending(state, { meta }) {
  return {
    ...state,
    columns: mergeArraysByKey([defaultColumns.filter(({ key }) => key !== 'tags' || meta?.showTags), state.columns], 'key'),
    rows: [],
    loaded: false,
  };
}
