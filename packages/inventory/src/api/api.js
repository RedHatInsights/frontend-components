...
export async function getEntities(
  items,
  { controller, hasItems, filters, per_page: perPage, page, orderBy, orderDirection, fields = { system_profile: ['operating_system'] }, ...options },
  showTags
) {
  if (hasItems && items.length > 0) {
    let data = await hosts.apiHostGetHostById(items, undefined, undefined, undefined, undefined, undefined, {
      cancelToken: controller && controller.token,
    });

    if (fields && Object.keys(fields).length) {
      try {
        const result = await hosts.apiHostGetHostSystemProfileById(items, undefined, undefined, undefined, undefined, undefined, undefined, {
          cancelToken: controller && controller.token,
        ...
        data = {
          ...data,
          results: mergeArraysByKey([data?.results, result?.results || []], 'id'),
        };
      } catch (e) {
        console.error(e);
      }
    ...
    return hosts
      .apiHostGetHostList(
        ...
        orderDirection,
        filters.staleFilter,
        [...constructTags(filters.tagFilters), ...(options.tags || [])],
...
export function getTags(systemId, search, { pagination } = { pagination: {} }) {
  return hosts.apiHostGetHostTags(systemId, pagination.perPage || 10, pagination.page || 1, undefined, undefined, search);
}

export function getAllTags(search, { filters, pagination, ...options } = { pagination: {} }) {
  const { tagFilters, staleFilter, registeredWithFilter } = filters ? filters.reduce(filtersReducer, defaultFilters) : defaultFilters;
  return tags.apiTagGetTags(
    [...(tagFilters ? constructTags(tagFilters) : []), ...(options.tags || [])],
    'tag',
    'ASC',
