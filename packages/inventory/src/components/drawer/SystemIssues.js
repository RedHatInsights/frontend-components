    ...
    <Stack hasGutter className="ins-c-inventory__drawer-system-issues">
      ...
      <StackItem>
        <Level>
          <LevelItem>
            <Stack hasGutter>
              <StackItem className="ins-c-inventory__drawer--title">Applicable CVEs</StackItem>
              <StackItem>
                {cve?.isLoaded ? (
                  <Level hasGutter className="ins-c-inventory__drawer-cve">
                    <LevelItem className="ins-m-critical">
                      <a href={`/insights/vulnerability/systems/${systemId}?impact=7`}>{cve?.critical?.count || 0} critical</a>
                    </LevelItem>
                    <LevelItem className="ins-m-important">
                      <a href={`/insights/vulnerability/systems/${systemId}?impact=5`}>{cve?.important?.count || 0} important</a>
                    </LevelItem>
                    <LevelItem className="ins-m-moderate">
                      <a href={`/insights/vulnerability/systems/${systemId}?impact=4`}>{cve?.moderate?.count || 0} moderate</a>
                    </LevelItem>
                    <LevelItem className="ins-m-low">
                      <a href={`/insights/vulnerability/systems/${systemId}?impact=2`}>{cve?.low?.count || 0} low</a>
                    </LevelItem>
                  </Level>
                ) : (
          ...
          <LevelItem>
            <Stack hasGutter>
              <StackItem className="ins-c-inventory__drawer--title">Applicable advisories</StackItem>
              <StackItem>
                {patch?.isLoaded ? (
      ...
      <StackItem>
        {advisor?.isLoaded ? (
          <span>
            <span className="ins-m-critical">{advisor?.criticalCount?.length || 0} critical</span> recommendations in{' '}
