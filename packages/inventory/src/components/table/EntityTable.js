...
import { Table as PfTable, TableBody, TableHeader, TableGridBreakpoint } from '@patternfly/react-table';
...
const EntityTable = ({
  ...
  const columnsRedux = useSelector(
    ...
    (next, prev) => next.every(({ key }, index) => prev.findIndex(({ key: prevKey }) => prevKey === key) === index)
  );
  ...
  useMemo(() => {
    if (typeof columnsProp === 'function') {
      ...
      const defaultColumnsFiltered = defaultColumns.filter(
        ({ key }) => (key === 'tags' && showTags) || (key !== 'tags' && !disabledColumns.includes(key))
      );
      columns.current = mergeArraysByKey(
        [typeof disableDefaultColumns === 'boolean' && disableDefaultColumns ? [] : defaultColumnsFiltered, columnsProp],
        'key'
      );
    } else {
      ...
      {loaded && cells ? (
        PfTable && (
          <PfTable
            ...
            cells={cells}
            rows={createRows(rows, columns.current, {
              actions,
              expandable,
              loaded,
            ...
            gridBreakPoint={columns.current?.length > 5 ? TableGridBreakpoint.gridLg : TableGridBreakpoint.gridMd}
            className="ins-c-entity-table"
            onSort={(event, index, direction) => {
              onSortChange(event, cells?.[index - Boolean(hasCheckbox) - Boolean(expandable)]?.key, direction, index);
            }}
            sortBy={{
