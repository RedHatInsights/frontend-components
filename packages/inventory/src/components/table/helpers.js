...
export const createColumns = (columns, hasItems, rows, isExpandable) =>
  columns?.map(({ props, transforms, cellFormatters, ...oneCell }) => ({
    ...
    transforms: [
      ...
      ...(hasItems || rows.length <= 0 || (props && props.isStatic) || transforms?.includes(sortable) || includesSortable(transforms)
        ? []
        : [sortable]),
    ],
    cellFormatters: [...(cellFormatters || []), ...(isExpandable ? [expandable] : [])],
  }));
