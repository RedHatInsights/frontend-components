...
describe('EntityTableToolbar', () => {
  ...
  describe('API', () => {
    ...
    describe('delete filter', () => {
      it('should dispatch action on delete filter', async () => {
        ...
        expect(onRefreshData).toHaveBeenCalledWith({
          filters: [{}, { filter: '', value: 'hostname_or_id' }, { staleFilter: ['stale'] }],
          page: 1,
          perPage: 50,
        });
        onRefreshData.mockClear();
        ...
        wrapper.update();
        expect(onRefreshData).toHaveBeenCalledWith({
          filters: [{}, { filter: '', value: 'hostname_or_id' }, { registeredWithFilter: [] }],
          page: 1,
          perPage: 50,
        });
      });
      ...
      it('should remove textual filter', async () => {
        ...
        wrapper.update();
        expect(onRefreshData).toHaveBeenCalledWith({ filters: [{ filter: '', value: 'hostname_or_id' }], page: 1, perPage: 50 });
      });
      ...
      it('should remove tag filter', async () => {
        ...
        wrapper.update();
        expect(onRefreshData).toHaveBeenCalledWith({ filters: [{ filter: '', value: 'hostname_or_id' }, { tagFilters: [] }], page: 1, perPage: 50 });
      });
    ...
    it('trim leading/trailling whitespace ', async () => {
      ...
      expect(state.entities.activeFilters).toMatchObject([{}, { filter: 'some-value', value: 'hostname_or_id' }]);
    });
  });
});
