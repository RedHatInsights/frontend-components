...
import { Dropdown, DropdownItem, DropdownPosition, DropdownToggle, Title, Button, Flex, FlexItem, Split, SplitItem } from '@patternfly/react-core';
        ...
        <SplitItem>
          {loaded ? (
            <Flex>
              ...
              {inventoryActions?.length > 0 && (
                <FlexItem>
                  <Dropdown
                    ...
                    toggle={<DropdownToggle onToggle={(isOpen) => setIsOpen(isOpen)}>Actions</DropdownToggle>}
                    isOpen={isOpen}
                    position={DropdownPosition.right}
                    dropdownItems={inventoryActions.map(({ title, ...action }, key) => (
                      <DropdownItem key={key} component="button" onClick={(event) => action.onClick(event, action, action.key || key)} {...action}>
                        {title}
                      </DropdownItem>
                    ))}
                  />
              ...
              <FlexItem>{showInventoryDrawer && <Button onClick={() => dispatch(toggleDrawer(true))}>Show more information</Button>}</FlexItem>
            </Flex>
          ) : (
      ...
      {isModalOpen && (
        <DeleteModal
          ...
          onConfirm={() => {
            ...
            deleteEntity([entity.id], entity.display_name, () => redirectToInventoryList(entity.id, onBackToListClick));
