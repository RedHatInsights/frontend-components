...
class RuleTable extends Component {
  ...
  render() {
    ...
    return (
      <section className="rhcs-c-rules-table">
        <PrimaryToolbar
        ...
        <Table
          ...
          cells={columns.map(({ transforms, ...column }) => ({
            ...column,
            transforms: [...(transforms || []), ...(!isLoading && sortBy && !column.disableSort ? [sortable] : [])],
          }))}
          rows={
            isLoading
        ...
        <TableToolbar isFooter>
          <Pagination {...pagination} widgetId="pagination-options-menu-bottom" variant={PaginationVariant.bottom} dropDirection="up" />
        </TableToolbar>
      </section>
    );
  }
