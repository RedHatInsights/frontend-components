...
export const calculateActiveFilters = (activeFilter, value, key) => value && value.length > 0 && activeFilter && ({
...
export const createRows = (data, columns, expanded, detail) => data.map((oneRule, ruleKey) => ([
    {
        cells: columns.map(({ selector }, cellKey) => ({
            title: typeof selector === 'function' ?
                selector(oneRule, ruleKey, cellKey) :
                get(oneRule, selector, 'Not found')
        })),
