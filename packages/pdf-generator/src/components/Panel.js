...
const Panel = ({ style, title, titleProps, contentProps, children, description, labels, ...props }) => (
  <View
    {...props}
    style={{
      ...style,
  ...
  >
    ...
    >
      {children}
      <View style={{ flex: 4 }}>
        <View style={[appliedStyles.panelLabels]}>{labels}</View>
        <Text>{customTitle(description)}</Text>
      </View>
    </View>
  </View>
);
