...
describe('EditButton with no access', () => {
  ...
  it('do not render with no permission - write permissions set to false', () => {
    const wrapper = mount(<EditButton onClick={onClick} link={link} writePermissions={false} />);
  ...
  it('render when write permissions are set to true', () => {
    const wrapper = mount(<EditButton onClick={onClick} link={link} writePermissions={true} />);
