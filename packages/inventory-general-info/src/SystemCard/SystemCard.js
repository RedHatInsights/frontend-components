...
const TitleWithPopover = ({ title, content }) => (
  <React.Fragment>
    <span>{title}</span>
    <Popover headerContent={<div>{title}</div>} bodyContent={<div>{content}</div>}>
      <Button variant="plain" aria-label={`Action for ${title}`} className="ins-active-general_information__popover-icon">
        ...
        <LoadingCard
          ...
          items={[
            ...(hasHostName
              ? [
                  {
                    title: <TitleWithPopover title="Host name" content="Name imported from the system." />,
                    value: entity.fqdn,
                    size: 'md',
                  },
                ]
            ...
            ...(hasDisplayName
              ? [
                  {
                    title: <TitleWithPopover title="Display name" content="System name displayed in an inventory list." />,
                    value: (
                      <Fragment>
                        {entity.display_name}
            ...
            ...(hasAnsibleHostname
              ? [
                  {
                    title: <TitleWithPopover title="Ansible hostname" content="Hostname that is used in playbooks by Remediations." />,
                    value: (
                      <Fragment>
                        {this.getAnsibleHost()}
              ...
              ? [
                  {
                    ...
                    target: 'sap_sids',
                    onClick: () => {
                      handleClick('SAP IDs (SID)', generalMapper(properties.sapIds, 'SID'));
                    },
                  },
                ]
              : []),
