...
describe('RadioFilter component', () => {
  ...
  it('renders with data', () => {
    ...
    cy.get('.pf-v6-c-menu ul.pf-v6-c-menu__list').children().should('have.length', 2);
  });

  it('renders with default value and placeholder', () => {
    cy.mount(<Radio {...config} value="val2" placeholder="baz" />);
    cy.get('.pf-v6-c-menu-toggle').click();
    cy.get('.pf-v6-c-menu ul.pf-v6-c-menu__list')
      .children()
      .eq(1)
      .within(() => {
        cy.get('.pf-v6-c-radio__input').should('be.checked');
      });
    cy.get('.pf-v6-c-menu-toggle__text').should('contain', 'baz');
  });
  ...
  it('onChange called', () => {
    ...
    cy.get('.pf-v6-c-menu-toggle').click();
    cy.get('.pf-v6-c-menu ul.pf-v6-c-menu__list').children().eq(1).click();
