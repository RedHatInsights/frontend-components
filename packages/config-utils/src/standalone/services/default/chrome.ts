...
export default ({ port }: { path?: string; port?: number }) => ({
...
export const registerChrome = ({
  ...
  app.use((req, res, next) => {
    ...
    if (req.method === 'GET' && ['', '.hmt', '.html'].includes(ext)) {
      ...
      // @ts-ignore
      function writeOrEnd(chunk: any, encoding: BufferEncoding, callback: ((error: Error | null | undefined) => void) | undefined, oldFn: any): any {
        ...
        if (typeof ctype === 'string' && ctype.includes('text/html') && !res.headersSent && !res.writableEnded) {
          ...
          if (typeof chunk === 'string') {
            ...
            chunk = chunk.replace(esiRegex, (_match, file) => {
              ...
              hasEsi = true;
              return fs.readFileSync(snippet, { encoding: 'utf-8' });
            });
      ...
      // @ts-ignore
      res.write = (chunk, encoding, callback) => writeOrEnd(chunk, encoding, callback, oldWrite) as any;
      // @ts-ignore
      res.end = (chunk, encoding, callback) => writeOrEnd(chunk, encoding, callback, oldEnd) as any;
    }

    next();
  });
  // Serve files which respect `keycloakUri` and `https`.
  app.get('(/beta)?/apps/chrome/*', (req, res, next) => {
