...
import realms from './default/keycloak/realm_export.json';
const internalRealms: { id: string; users: any[] }[] = realms;
// @ts-ignore
const users = internalRealms?.find((f) => f.id === 'redhat-external').users;
...
const backOfficeProxy = {
  register({ app }: { app: express.Application }) {
