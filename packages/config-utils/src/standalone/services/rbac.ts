...
function getRbacConfigFolder(env: string) {
...
const services = ({ port, env, assets }: { port: number; env: string; assets: { [key: string]: string } }) => ({
  redis: {
  ...
  rbac: {
    ...
    args: [
      ...
      `-v ${path.resolve(
        assets['rbac-config'],
        'configs',
        getRbacConfigFolder(env),
        'permissions'
      )}:/opt/app-root/src/rbac/management/role/permissions:ro`,
