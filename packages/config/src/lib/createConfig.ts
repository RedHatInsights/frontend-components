...
export interface CreateConfigOptions extends CommonConfigOptions {
  ...
  /** Custom path to an SSL certificate file (e.g. a .pem generated by mkcert). Used with sslKey. */
  sslCert?: string;
  /** Custom path to an SSL key file (e.g. a .pem generated by mkcert). Used with sslCert. */
  sslKey?: string;
}
    ...
    if (fs.existsSync(certFile) && fs.existsSync(keyFile)) {
      ...
      fecLogger(
        ...
        `Custom SSL certificate files not found:\n` +
          `  cert: ${certFile}${fs.existsSync(certFile) ? '' : ' (missing)'}\n` +
          `  key:  ${keyFile}${fs.existsSync(keyFile) ? '' : ' (missing)'}\n` +
      ...
      fecLogger(
        ...
        'Using self-signed certificate for dev server.\n' +
          ...
          'or provide custom paths via sslCert/sslKey in fec.config.js.',
      );
    }
  }
