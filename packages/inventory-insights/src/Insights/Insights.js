  ...
  fetchKbaDetails = async (reportsData) => {
    ...
    try {
      const kbaDetailsFetch = (
        await API.get(
          `https://access.redhat.com/hydra/rest/search/kcs?q=id:(${kbaIds.join(` OR `)})&fl=view_uri,id,publishedTitle&rows=${
            kbaIds.length
  ...
  buildRows = (activeReports, kbaDetails, filters, rows, searchValue = '', kbaLoading = false) => {
    const builtRows = flatten(
      activeReports.map((value, key) => {
        ...
        const isOpen = rows.length ? (entity.length ? entity[0].isOpen : false) : key === 0 ? true : false;
        const reportRow = [
          {
            rule,
            resolution,
                ...
                title: (
                  <div key={key} style={{ verticalAlign: 'top' }}>
                    <Tooltip
                      ...
                      content={
                        <span>
                          The <strong>likelihood</strong> that this will be a problem is {LIKELIHOOD_LABEL[rule.likelihood]}. The{' '}
                          <strong>impact</strong> of the problem would be &nbsp;{IMPACT_LABEL[rule.impact.impact]} if it occurred.
                        </span>
                      }
                    >
      ...
      <div className="ins-c-inventory-insights__overrides">
        ...
        {inventoryReportFetchStatus === 'fulfilled' &&
          (hideResultsSatelliteManaged ? (
            <MessageState
              ...
              text={
                <span key="satellite managed system">
                  Insights results can not be displayed for this host, as the &quot;Hide Satellite Managed Systems&quot; setting has been enabled by
                  an org admin.
                  <br />
            ...
            >
              <Button component="a" href="https://access.redhat.com/products/red-hat-insights#getstarted" target="_blank" variant="primary">
