...
Node and npm versions are managed by **nvm** (Node) and **Corepack** (npm):

- **Node**: version is pinned in `.nvmrc`. Activate it with nvm:
  ```bash
  nvm install   # first time — installs the pinned version
  nvm use       # subsequent times — switches to the pinned version
  ```
- **npm**: exact version is pinned in the `packageManager` field of `package.json`. Activate it with Corepack (bundled with Node 14.19+):
  ```bash
  corepack enable npm   # installs the npm shim (one-time per machine)
  corepack install      # activates the exact pinned npm version from packageManager
  ```

> **Why both?** `.nvmrc` controls Node; `packageManager` + Corepack controls the exact npm build that CI uses. `engine-strict=true` in `.npmrc` will hard-fail if either is out of range, so always set up both before running `npm install`.

### Installation

```bash
npm install        # local development — updates package-lock.json if needed
npm ci             # clean install — matches package-lock.json exactly (preferred in CI)
```
